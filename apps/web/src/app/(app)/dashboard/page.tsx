...
const KPI = [
  { label: "Aktive Sitzungen",  value: "20",          sub: "in 8 Warengruppen",  delta: "+2 heute",  sign: 1  },
  { label: "Tagesumsatz",       value: "€ 48,2 Mio.", sub: "vs. gestern",        delta: "+4,7 %",    sign: 1  },
  { label: "Registrierte Teilnehmer", value: "1.240", sub: "in 31 EU-Ländern",   delta: "+12 neu",   sign: 1  },
  { label: "Abschlüsse heute",  value: "843",          sub: "Ø 57 t / Deal",      delta: "+63 vs. Ø", sign: 1  },
];
...
export default function DashboardPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 32, fontFamily: SANS }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", paddingBottom: 20, borderBottom: "1px solid #e8e8e8" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ textAlign: "right" }}>
            <p style={{ fontSize: 10, color: "#bbb", margin: 0, letterSpacing: "0.08em", textTransform: "uppercase" }}>Handelsphase</p>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4, justifyContent: "flex-end" }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: "#166534", fontFamily: MONO }}>GEÖFFNET</span>
            </div>
          </div>
          <div style={{ width: 1, height: 32, backgroundColor: "#e8e8e8" }} />
          <div style={{ textAlign: "right" }}>
            <p style={{ fontSize: 10, color: "#bbb", margin: 0, letterSpacing: "0.08em", textTransform: "uppercase" }}>Sitzungsende</p>
      ...
      {/* ── KPI-Zeile ──────────────────────────────────────────────── */}
      <div>
        <SectionHeader title="Marktübersicht — Heute" />
      ...
      {/* ── Chart + Top-Sektionen ──────────────────────────────────── */}
      <div>
        <SectionHeader title="Handelsverlauf" />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 1, backgroundColor: "#dde1e8" }}>
          ...
          <div style={{ backgroundColor: "#fff", padding: "20px 24px" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
              <div>
                <p style={{ fontSize: 13, fontWeight: 600, color: "#0d1b2a", margin: 0 }}>Tagesumsatz — stündlicher Verlauf</p>
                <p style={{ fontSize: 11, color: "#aaa", marginTop: 4, fontFamily: MONO }}>EUR · Spotmarkt Frankfurt · Echtzeit</p>
              </div>
          ...
          <div style={{ backgroundColor: "#fff", padding: "20px 24px", display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 10, color: "#bbb", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>nach Volumen</span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 0, flex: 1 }}>
              {topCategories.map(({ id, label, volume, change, up, volumeNum }, idx) => (
                <Link key={id} href={`/trading?category=${id}`} style={{ textDecoration: "none" }}>
                  <div style={{ padding: "9px 0", borderBottom: idx < topCategories.length - 1 ? "1px solid #f5f5f5" : "none" }}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                      ...
                      <span style={{ fontSize: 12, color: "#333", flex: 1, fontWeight: 500 }}>{label}</span>
      ...
      <div>
        ...
        />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 1, backgroundColor: "#dde1e8" }}>
          {CATEGORIES.map(({ id, label, sub, sessions, change, up, volume, Icon }) => (
            <Link key={id} href={`/trading?category=${id}`} style={{ textDecoration: "none" }}>
              <div style={{ backgroundColor: "#fff", padding: "16px 20px", cursor: "pointer", height: "100%", boxSizing: "border-box" }}
                ...
                <p style={{ fontSize: 13, fontWeight: 600, color: "#0d1b2a", margin: 0 }}>{label}</p>
                ...
                <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px solid #f5f5f5", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    ...
                    <span style={{ fontSize: 10, color: "#aaa" }}>{sessions} Sitz.</span>
                  </div>
                </div>
              </div>
            </Link>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ backgroundColor: "#fafafa", borderBottom: "1px solid #eaeaea" }}>
                {[
                  { label: "Sitzung",  align: "left"   },
                  { label: "Sektion", align: "left"   },
                  { label: "Zeitfenster", align: "left"   },
                  { label: "Aufträge", align: "right"  },
            ...
            <tbody>
              {ACTIVE_SESSIONS.map((s, i) => {
                ...
                const stLabel   = isOpen ? "Offen" : isSell ? "Nur Verkauf" : s.status;
